...
CShadow::CShadow(float _fX, float _fY)
{
	m_tInfo.fX = _fX;
	m_tInfo.fY = _fY;
}
...
int CShadow::Update(void)
{
	...
	m_tInfo.fY = (float)(m_pTarget->Get_Info().fY + m_pTarget->Get_Info().fCY * 0.5);
	Update_Rect();
	return OBJ_NOEVENT;
}
...
void CShadow::Set_Shawdow(TCHAR* _key, float _fCX, float _fCY)
{
	m_hMemDC = CBmpMgr::Get_Instance()->Find_Image(_key);
	m_tInfo.fCX = _fCX;
	m_tInfo.fCY = _fCY;
	Update_Rect();
}
