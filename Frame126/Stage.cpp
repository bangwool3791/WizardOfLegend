...
void CStage::Initialize(void)
{
	...
	CScrollMgr::Get_Instance()->Set_ScrollX(-FX_PLAYER + WINCX * 0.5);
	CScrollMgr::Get_Instance()->Set_ScrollY(-FY_PLAYER + WINCY * 0.5);

	CObjMgr::Get_Instance()->AddObject(OBJ_KEY, OBJ_PLAYER, CAbstactFactory<CPlayer>::Create(FX_PLAYER, FY_PLAYER));
	CObjMgr::Get_Instance()->AddObject(UI_KEY, UI_TELEPORT, CAbstactFactory<CTelePort>::Create(FX_PLAYER, FY_PLAYER));
	CObjMgr::Get_Instance()->AddObject(UI_KEY, UI_TELEPORT, CAbstactFactory<CTelePort>::Create(FX_TELEPORT_BOSS, FY_TELEPORT_BOSS));

	CObjMgr::Get_Instance()->AddObject(UI_KEY, UI_SHOP, CAbstactFactory<CShop>::Create(FX_SHOP_NPC, FY_SHOP_NPC));

	dynamic_cast<CTelePort*>(CObjMgr::Get_Instance()->Get_Target(UI_KEY, UI_TELEPORT, nullptr))->Set_Opt(CTelePort::TELE_OPT_BOSS);
	CTileMgr::Get_Instance()->Initialize();
	CTileMgr::Get_Instance()->Load_Process();

	InitializeSound();
}
void CStage::InitializeSound(void)
{
...
void CStage::Release(void)
{
	...
	CObjMgr::Get_Instance()->Delete_ID(UI_KEY, UI_TELEPORT);
	CObjMgr::Get_Instance()->Delete_ID(UI_KEY, EFFECT_BOTTOM_HOLE);
	CObjMgr::Get_Instance()->Delete_ID(UI_KEY, UI_MIDDLE_BOSS_BAR);
	CObjMgr::Get_Instance()->Delete_ID(UI_KEY, UI_BOSS_BAR);
	CObjMgr::Get_Instance()->Delete_ID(UI_KEY, UI_ITEM);
	CObjMgr::Get_Instance()->Delete_ID(UI_KEY, UI_SHOP);
	CObjMgr::Get_Instance()->Delete_ID(UI_KEY, UI_TELEPORT);
